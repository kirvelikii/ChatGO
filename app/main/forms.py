...
class EditProfileForm(FlaskForm):
    ...
    about_me = TextAreaField(_l('Немного о себе'),
    ...
    def validate_username(self, username):
        if username.data != self.original_username:
            ...
            if user is not None:
                raise ValidationError(_('Пожалуйста, выберите другое имя пользователя'))
...
class PostForm(FlaskForm):
    post = TextAreaField(_l('Напишите что-нибудь'), validators=[DataRequired()])
